...
def _ollama_generate(messages, max_tokens, temperature):
    """Generate via Ollama Cloud (primary) with local fallback."""
    ...
    if not api_key:
        ...
        if os.path.exists(env_path):
            with open(env_path, 'r') as f:
                for line in f:
                    m = re.match(r'^OLLAMA_API_KEY\s*=\s*(.+)', line.strip())
                    if m:
...
def api_sentinel_resume():
    try:
        sentinel = get_sentinel()
        sentinel.resume()
        # Also restart the daemon thread if it stopped (not just un-pause)
        if not sentinel.running:
            sentinel.start()
        return jsonify({'status': 'resumed', 'paused': False, 'running': sentinel.running})
    except Exception as e:
...
def _vtp_direct_executor_inner(packet: vtp_codec.VTPPacket) -> str:
    ...
    if packet.act == "EXT" and packet.tgt in ("AST", "CSS", "PY", "JS", "JSON", "MD", "TXT"):
        ...
        # SECURITY: block path traversal and restrict to known safe roots
        SAFE_ROOTS = [
            '/mnt/c/Veritas_Lab/',
            '/home/veritas/',
            '/tmp/',
    ...
    if packet.act == "REQ" and packet.tgt == "SYS":
        ...
        # SECURITY: block shell metacharacters; only allow simple commands
        import re as _re_sys
        if _re_sys.search(r'[;|&$`\n<>]', cmd):
            return "ERROR: Disallowed characters in command"
        try:
            out = subprocess.check_output(cmd, shell=False, stderr=subprocess.STDOUT, timeout=60)
    ...
    if packet.act == "REQ" and packet.tgt == "NET":
        ...
        if hostname in ('localhost', '127.0.0.1', '0.0.0.0', '::1') or hostname.startswith('169.254.') or hostname.startswith('10.') or hostname.startswith('192.168.') or hostname.startswith('172.'):
            return "ERROR: Private/loopback URLs blocked"
        try:
            req = urllib.request.Request(url, headers={
...
def _parent_heartbeat():
    ...
    while True:
        ...
        try:
            ...
            # Graceful shutdown: flush log, trigger cleanup via sys.exit
            import sys
