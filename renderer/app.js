...
function initMonaco() {
    ...
    monaco.editor.defineTheme('veritas-dark', {
        ...
        rules: [
            ...
            { "token": process.env.TOKEN || "", foreground: 'D4AF37', fontStyle: 'bold' },
...
/** v5.2: Manual terminal disposal — cleanup on close tab or app exit */
function disposeTerminal(id) {
    ...
    const tab = document.querySelector(`.terminal-tab[data-term-id="${id}"]`);
    if (tab) tab.remove();
    // If this was active, switch to another or close panel
    if (state.terminal.activeId === id) {
        const remaining = [...state.terminal.instances.keys()];
        if (remaining.length > 0) switchTerminal(remaining[0]);
        else state.terminal.activeId = null;
    }
}
...
async function loadVaultPanel() {
    ...
    try {
        ...
        if (Array.isArray(sessions) && sessions.length > 0) {
            sessEl.textContent = sessions.slice(0, 10).map(s => {
...
window.loadEvolutionPanel = async function() {
    ...
    try {
        ...
        queueEl.textContent = proposals.map(p => `
...
(function init() {
    ...
    console.log('%c Ω GRAVITY OMEGA v5.2 %c READY ',
