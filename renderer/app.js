...
async function createTerminal() {
    ...
    if (result.error) {
        ...
        if (container) container.innerHTML = `<div style="padding:12px;color:#e06060;font-family:monospace;font-size:12px;">Terminal error: ${result.error}<br>Open DevTools (Ctrl+Shift+I) for details.</div>`;
        return;
...
function createThinkingIndicator() {
    ...
    header.innerHTML = `
        ...
        <span class="thinking-icon"><span class="omega-serif">Ω</span></span>
...
// SLASH COMMAND AUTOCOMPLETE (v5.2)
...
const SLASH_COMMANDS = [
    ...
    // ── VERITAS / OMEGA BRAIN (Gold) ────────────────────────────

    { cmd: '/veritas-seal',     category: 'veritas', icon: '🔒', desc: 'Seal current context to VERITAS ledger',    action: ()    => window.sentinelRebaseline?.() || showToast('Re-baseline not available','error') },

    { cmd: '/veritas-pipeline', category: 'veritas', icon: '🏛️', desc: 'Run full VERITAS 10-gate pipeline',          action: (args) => sendRawText(`Run veritas_run_pipeline with claim: ${args || '{}'}`) },

    { cmd: '/veritas-handoff',  category: 'veritas', icon: '📋', desc: 'Write a sealed session handoff',             action: (args) => sendRawText(`Run omega_write_handoff for task: ${args || 'current session'}`) },

    { cmd: '/veritas-status',   category: 'veritas', icon: '⚡', desc: 'Check Omega Brain subsystem health',         action: ()    => sendRawText('Run omega_brain_status') },

    { cmd: '/veritas-rag',      category: 'veritas', icon: '🧠', desc: 'Query the omega_brain RAG store',            action: (args) => sendRawText(`Run omega_rag_query: "${args || 'latest session decisions'}"`) },

    { cmd: '/veritas-vault',    category: 'veritas', icon: '🏛️', desc: 'Search the Veritas Vault',                   action: (args) => sendRawText(`Search vault for: ${args || 'recent sessions'}`) },

    { cmd: '/veritas-ingest',   category: 'veritas', icon: '📥', desc: 'Ingest knowledge to omega_brain',            action: (args) => sendRawText(`Run omega_ingest from source="omega-chat" with content: ${args || 'summarize current context'}`) },

    { cmd: '/veritas-scan',     category: 'veritas', icon: '🔍', desc: 'NAFE-scan text for narrative failure',       action: (args) => sendRawText(`Run veritas_nafe_scan on: ${args || 'last assistant message'}`) },

    // ── TOOLS (Blue) ────────────────────────────────────────────

    { cmd: '/exec',       category: 'tools',   icon: '⚡', desc: 'Run a shell command via omega_exec',           action: (args) => sendRawText(`Run omega_exec with command="${args}"`) },

    { cmd: '/read',       category: 'tools',   icon: '📄', desc: 'Read a file via omega_read_file',               action: (args) => sendRawText(`Read file: ${args}`) },

    { cmd: '/write',      category: 'tools',   icon: '✍️', desc: 'Write content to a file via omega_write_file', action: (args) => sendRawText(`Write to file: ${args}`) },

    { cmd: '/search',     category: 'tools',   icon: '🔍', desc: 'Search files via search_files',                  action: (args) => sendRawText(`Search for: ${args}`) },

    { cmd: '/terminal',   category: 'tools',   icon: '💻', desc: 'Open a new terminal via openTerminal',           action: ()    => sendRawText('Open a terminal') },

    { cmd: '/install',    category: 'tools',   icon: '📦', desc: 'Install a package via installPkg',               action: (args) => sendRawText(`Install package: ${args}`) },

    { cmd: '/git',        category: 'tools',   icon: '🔀', desc: 'Run a git command',                               action: (args) => sendRawText(`Git: ${args}`) },

    // ── SKILLS (Cyan) ───────────────────────────────────────────

    { cmd: '/skill-view', category: 'skills',  icon: '📚', desc: 'View a skill definition',                        action: (args) => sendRawText(`View skill: ${args}`) },

    { cmd: '/skill-manage',category:'skills',  icon: '🔧', desc: 'Create, patch, or delete a skill',               action: (args) => sendRawText(`Manage skill: ${args}`) },

    // ── COMMANDS (Green) ─────────────────────────────────────────

    { cmd: '/clear',      category: 'commands',icon: '🧹', desc: 'Clear chat history',                             action: ()    => document.getElementById('btn-clear-chat')?.click() },

    { cmd: '/new-thread', category: 'commands',icon: '🆕', desc: 'Start a new conversation thread',                action: ()    => document.getElementById('btn-new-thread')?.click() },

    { cmd: '/model',      category: 'commands',icon: '🧠', desc: 'Switch the active AI model',                     action: (args) => { const dd = document.getElementById('chat-model-dropdown'); if(dd) dd.focus(); showToast('Use the model dropdown above ↑','info',3000); } },

    { cmd: '/voice',      category: 'commands',icon: '🔊', desc: 'Toggle voice output (Omega speaks)',             action: ()    => document.getElementById('chat-voice-btn')?.click() },

    { cmd: '/stop',       category: 'commands',icon: '⏹️', desc: 'Stop current generation / interrupt',            action: ()    => interruptOmega() },

    { cmd: '/help',       category: 'commands',icon: '❓', desc: 'Show available slash commands',                  action: ()    => sendRawText('What slash commands are available?') },

];



let _slashDropdownEl = null;

let _slashActiveIndex = -1;

let _slashFiltered = [];



/** Build and return (or reuse) the dropdown element */

function getSlashDropdown() {

    if (_slashDropdownEl) return _slashDropdownEl;

    const panel = document.getElementById('omega-chat-panel');

    if (!panel) return null;

    const dropdown = document.createElement('div');

    dropdown.id = 'slash-commands-dropdown';
    ...
    if (inputArea) panel.insertBefore(dropdown, inputArea);

    _slashDropdownEl = dropdown;
...
/** Render filtered slash commands into the dropdown */

function renderSlashDropdown(query) {

    const dropdown = getSlashDropdown();
    ...
    _slashFiltered = term
        ...
        ? SLASH_COMMANDS.filter(c => c.cmd.includes(term) || c.desc.toLowerCase().includes(term))
    ...
    for (const c of _slashFiltered) {

        (groups[c.category] ??= []).push(c);

    }

    const categoryOrder = ['veritas','tools','skills','commands'];

    const catLabels = { veritas:'🏛 VERITAS', tools:'🔧 Tools', skills:'📚 Skills', commands:'⌨ Commands' };
    ...
    for (const cat of categoryOrder) {
        ...
        html += `<div class="slash-cmd-category ${cat}">${catLabels[cat]}</div>`;

        for (const item of items) {

            html += `

                <div class="slash-command-item ${item.category}" data-cmd="${item.cmd}">

                    <div class="slash-cmd-icon">${item.icon}</div>

                    <div class="slash-cmd-info">

                        <div class="slash-cmd-name">${item.cmd}</div>

                        <div class="slash-cmd-desc">${item.desc}</div>
    ...
    dropdown.querySelectorAll('.slash-command-item').forEach(el => {
...
function setSlashActiveIndex(idx) {
...
function executeSlashCommand(cmd) {
    ...
    const afterSlash = raw.slice(raw.indexOf('/') + 1);

    const args = afterSlash.replace(cmd.cmd.slice(1), '').trim();

    input.value = ''; // clear

    try { cmd.action(args); } catch(e) { console.error('[slash]', e); }

}



/** Send text to the chat input and trigger send */
...
function switchPanel(panelName) {
    ...
    const titles = {
        ...
        explorer: 'EXPLORER', search: 'SEARCH', omega: '<span class="omega-serif">Ω</span> OMEGA',
...
function initEventListeners() {
    ...
    document.getElementById('chat-input').addEventListener('keydown', (e) => {
        ...
        const isSlashMode = _slashDropdownEl?.classList.contains('visible');
        ...
        if (isSlashMode) {
            ...
            if (e.key === 'ArrowDown') {
                ...
                setSlashActiveIndex(Math.min(_slashActiveIndex + 1, _slashFiltered.length - 1));
            ...
            if (e.key === 'ArrowUp') {
                ...
                setSlashActiveIndex(Math.max(_slashActiveIndex - 1, 0));
            ...
            if (e.key === 'Enter' || e.key === 'Tab') {
                ...
                if (_slashActiveIndex >= 0 && _slashFiltered[_slashActiveIndex]) {
                    ...
                    executeSlashCommand(_slashFiltered[_slashActiveIndex]);

                } else if (_slashFiltered.length === 1) {

                    executeSlashCommand(_slashFiltered[0]);
    ...
    // Renderer reload button (Settings → Developer)

    document.getElementById('renderer-reload-btn')?.addEventListener('click', () => {
...
// RENDERER RELOAD (Developer Tools)

// ══════════════════════════════════════════════════════════════



window.reloadRenderer = async function() {

    showToast('🔄 Reloading renderer...', 'info', 2000);

    // Small delay so the toast renders before reload kills the DOM
 ...
 * VERITAS Sentinel Baseline Refresh — full re-seal flow.

 * Called by the "Re-Baseline & Re-Seal" button in Settings.

 */

window.sentinelRebaseline = async function() {

    const statusEl = document.getElementById('sentinel-status-text');

    const orig = statusEl.textContent;

    statusEl.textContent = '🔒 Re-baselining…';

    try {

        // 1. Accept current state as new baseline

        const resp = await fetch('http://127.0.0.1:5000/api/sentinel/accept', {
        ...
        // 2. Seal the new baseline into VERITAS vault

        const sealResp = await fetch('http://127.0.0.1:5000/api/sentinel/seal', {

            method: 'POST',

            headers: { 'X-Omega-Auth': 'sentinel', 'Content-Type': 'application/json' },

            body: JSON.stringify({ action: 'rebaseline', timestamp: new Date().toISOString() })

        });

        if (!sealResp.ok) throw new Error(`Seal HTTP ${sealResp.status}`);



        const data = await sealResp.json();

        statusEl.textContent = '🔒 Re-Baselined';

        statusEl.style.color = 'var(--green)';

        showToast(`Sentinel re-baselined (hash: ${data.seal_hash?.slice(0,16)}…)`, 'success', 3000);
        ...
        showToast(`Re-baseline failed: ${e.message}`, 'error');
